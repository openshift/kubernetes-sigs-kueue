...
<<<<<<< HEAD:pkg/controller/jobframework/base_webhook_test.go
...
=======
	...
	testingutil "sigs.k8s.io/kueue/pkg/util/testingjobs/mxjob"
)
...
func TestDefault(t *testing.T) {
	testcases := map[string]struct {
		...
		manageJobsWithoutQueueName bool
		want                       *kftraining.MXJob
	}{
		"update the suspend field with 'manageJobsWithoutQueueName=false'": {
			job:  testingutil.MakeMXJob("job", "default").Queue("queue").Suspend(false).Obj(),
			want: testingutil.MakeMXJob("job", "default").Queue("queue").Obj(),
		},
		"update the suspend field 'manageJobsWithoutQueueName=true'": {
			job:                        testingutil.MakeMXJob("job", "default").Suspend(false).Obj(),
			manageJobsWithoutQueueName: true,
			want:                       testingutil.MakeMXJob("job", "default").Obj(),
>>>>>>> 18a07172 (Revert "Rebase kueue"):pkg/controller/jobs/kubeflow/jobs/mxjob/mxjob_webhook_test.go
		},
	}
...
=======
			w := &MXJobWebhook{manageJobsWithoutQueueName: tc.manageJobsWithoutQueueName}
>>>>>>> 18a07172 (Revert "Rebase kueue"):pkg/controller/jobs/kubeflow/jobs/mxjob/mxjob_webhook_test.go
			if err := w.Default(context.Background(), tc.job); err != nil {
				t.Errorf("set defaults to a kubeflow.org/mxjob by a Defaulter")
			}
